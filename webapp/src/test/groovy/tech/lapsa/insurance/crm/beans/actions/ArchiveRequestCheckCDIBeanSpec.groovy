...
import tech.lapsa.insurance.crm.beans.actions.ArchiveRequestCDIBean.ArchiveRequestCheckCDIBean
import tech.lapsa.insurance.crm.beans.actions.CancelRequestCDIBean.CancelRequestCheckCDIBean
import tech.lapsa.insurance.crm.rows.RequestRow

class ArchiveRequestCheckCDIBeanSpec extends Specification {

    FacesContext facesContext
    ExternalContext externalContext


    static allowedProgressStatus = FINISHED
    ...
    @Unroll
    def "Archive Request allowance is '#expAllowed' on role '#role'"() {
	given:
	...
	def bean = new ArchiveRequestCheckCDIBean(rrs: rrs)
    ...
    @Unroll
    def "Archive Request allowance is '#expAllowed' on insuranfce request status '#insuranceRequestStatus'"() {
	given:
    ...
    @Unroll
    def "Archive Request allowance is '#expAllowed' on progress status '#progressStatus'"() {
	given:
    ...
    @Unroll
    def "Archive Request allowance is '#expAllowed' on selection of #number rows"() {
	given:
    ...
    @Unroll
    def "Archive Request allowance is '#expAllowed' on no multiple selection where of row's status is '#insuranceRequestStatus'"() {
	given:
