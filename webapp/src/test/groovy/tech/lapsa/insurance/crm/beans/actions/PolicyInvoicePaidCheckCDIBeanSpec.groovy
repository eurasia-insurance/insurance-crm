...
import tech.lapsa.insurance.crm.beans.actions.PolicyInvoicePaidCDIBean.PolicyInvoicePaidCheckCDIBean
import tech.lapsa.insurance.crm.rows.RequestRow

class PolicyInvoicePaidCheckCDIBeanSpec extends Specification {

    FacesContext facesContext
    ExternalContext externalContext

    static allowedProgressStatus = ON_PROCESS
    static allowedInsuranceRequestStatus = POLICY_ISSUED
    ...
    @Unroll
    def "Policy Invoice Paid allowance is '#expAllowed' on role '#role'"() {
	given:
	...
	def bean = new PolicyInvoicePaidCheckCDIBean(rrs: rrs)
    ...
    @Unroll
    def "Policy Invoice Paid allowance is '#expAllowed' on insuranfce request status '#insuranceRequestStatus'"() {
	given:
    ...
    @Unroll
    def "Policy Invoice Paid allowance is '#expAllowed' on progress status '#progressStatus'"() {
	given:
    ...
    @Unroll
    def "Policy Invoice Paid allowance is '#expAllowed' on selection of #number rows"() {
	given:
